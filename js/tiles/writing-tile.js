...
(function () {
  ...
  function updateSelectedButton(level) {
    var buttons = document.querySelectorAll('#writingTile .btn-level');
    for (var i = 0; i < buttons.length; i++) {
      buttons[i].classList.remove('btn-level-selected');
    }
    if (level !== null) {
      var targetBtn = document.querySelector('#writingTile .btn-level[data-level="' + level + '"]');
      if (targetBtn) {
  ...
  function emitProofPointAdded() {
    if (state.points.length > 0) {
      document.dispatchEvent(new CustomEvent('proofPointAdded', {
        detail: { type: 'writing', level: state.points[state.points.length - 1].level, time: state.currentTime }
      }));
    }
  }
  ...
  function registerProofWithEngine() {
    if (window.__ProofTrustEngine) {
      window.__ProofTrustEngine.registerProof('writing', state.points, 3);
    }
  }
